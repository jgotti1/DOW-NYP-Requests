...
function DataTable({ onRowClick }) {
  const { rowDataDow, filtersDow, searchTermDow } = useFiltersDow(); 
...
const isPastOrToday = (requestNeededDate) => {
  ...
  const filteredRowData = rowDataDow.filter((row) => {
    const term = searchTermDow.toLowerCase();
    ...
    const matchesStatus = (filtersDow.completed && row.status === "Complete") || (filtersDow.inProgress && row.status === "In Progress") || (filtersDow.newRequests && row.status === "New");
