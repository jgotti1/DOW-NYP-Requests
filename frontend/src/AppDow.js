...
function App({ admin }) {
  ...
  const handleCopy = async (id) => {
    ...
    try {
      ...
      const { data: original } = await axios.get(`${baseUrl}dowrequests/${id}`);
      const payload = toPayload(original);
      const postRes = await axios.post(`${baseUrl}dowrequests/`, payload);

      const newId = postRes?.data?.requestId ?? postRes?.data?.id ?? postRes?.data?._id ?? postRes?.data?.insertId;

      if (!newId) {
        console.log("POST /dowrequests/response:", postRes.data, postRes.headers);
        throw new Error("Backend did not return new record id");
      }

      // Get canonical created record
      const { data: createdRecord } = await axios.get(`${baseUrl}dowrequests/${newId}`);
      ...
      // refresh table
      const { data: all } = await axios.get(`${baseUrl}dowrequests/all`);
      setRowDataDow(all);
    } catch (err) {
    ...
    <div className="App">
      {admin ? (
        <div className="sticky-header">
          <div className="head-container-dow">
        ...
        <div className="sticky-header">
          <div className="head-container-dow">
            <div className="center-content">
              <img src="/images/dowlogo.png" alt="DOW LOGO" />
